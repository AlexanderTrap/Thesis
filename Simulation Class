...
path = 'C:\\Users\\Masterstudent\\PycharmProjects\\Thesis_Alexander_Trap\\Figuren'

class SimulationStochasticIC(object):
    ''' Class for the simulation of de expecation value of sigma_x of various stochastic inititial MPS
    ==========
    dB: max bond dimension of ground state, when 1 it's just de spins pointed in up direction on the
        Bloch sphere
    ...
    stochastic_MPS:
        returns 'psip', a stochastic ground state-like MPS
    plot:
        plots the graph and can save the graph
    '''

    def __init__(self, L, dB, chi, std, hx, hz, dt, T):
        self.dB = dB    #max bond dimension for ground state? This is always 2.
        self.L = L
    ...
    def time_evolution_correct(self):
        '''
        ...
        tebd_params = {
                        ...
                        'order': 4,
                        'verbose': 1,
                        'trunc_params': {'chi_max': 2**(self.L//2), 'svd_min': 1.e-12}
        }
    ...
    def time_evolution_chi(self):
        '''
        :return: time evolution of sigma for self.chi
        '''
        psi_corr = self.psi0.copy()
    ...
    def plot(self, savefig):
        ...
        :return: plot
        '''
        self.time_evolution_correct()
        self.time_evolution_chi()
    ...
    def make_noise_chi_greater_than_2(self, reps):
        ...
        cut = math.ceil(np.log(self.chi) / np.log(2))     #for lets say a chi=8, the chi-list will look like this [2,4,8,8,8,4,2]
